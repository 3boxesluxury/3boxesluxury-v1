...
import { Search, ShoppingCart, Package, Menu, X, LogIn, LogOut, User, Shield, Gift, Sparkles, Smartphone, Download, ExternalLink, Home } from 'lucide-react';
...
export function Header() {
  ...
  const handleSearch = useCallback(
    (e: React.FormEvent) => {
      e.preventDefault();
      // IMPORTANT: setView('home') must be called BEFORE setSearch,
      // because setView('home') clears searchQuery in the store.
      // If we set search first, then setView('home') wipes it out.
      setView('home');
      setSearch(localSearch);
    },
    [localSearch, setSearch, setView]
  );
        ...
        <div className="flex h-36 items-center justify-between gap-4">
          ...
          <div className="flex items-center flex-shrink-0">
            ...
            >
              <div className="logo-flashy">
                <Image
                  ...
                  width={140}
                  height={140}
                  className="h-[140px] w-auto sm:h-[150px]"
                  priority
                />
              </div>
            </button>
            ...
            <h1
              ...
              className="gold-shimmer text-xl font-bold tracking-widest sm:text-2xl hidden sm:block whitespace-nowrap cursor-pointer hover:text-amber-400 transition-colors ml-2 mr-2"
            >
              3 BOXES LUXURY
            </h1>
            ...
            <Button
              ...
              className="hidden sm:flex items-center gap-1.5 text-amber-200/80 hover:bg-amber-900/20 hover:text-amber-400 border border-amber-900/20 hover:border-amber-600/40 px-3 py-1.5"
            >
          ...
          {/* Search Bar - Desktop */}
          <form onSubmit={handleSearch} className="hidden flex-1 max-w-md md:flex ml-4">
            <div className="relative w-full">
              ...
              <Input
                ...
                className="w-full h-10 border-amber-900/40 bg-stone-900/80 pl-9 pr-4 text-amber-50 text-sm placeholder:text-amber-200/50 focus:border-amber-600/60 focus:ring-amber-600/30 focus-visible:ring-amber-600/20"
                style={{ backgroundColor: 'rgba(28, 25, 23, 0.85)', color: '#fef3c7' }}
              />
              {localSearch && (
                <button
                  type="button"
          ...
          <div className="flex items-center gap-2">
            {/* Get App Button — triggers PWA install prompt or opens installed app */}
            <Button
              variant="outline"
              size="sm"
              onClick={async () => {
            ...
            >
              {isInstalled ? (
                ...
                <>
                  ...
                  <span className="text-xs font-medium">{canInstall ? 'Install App' : 'Get App'}</span>
                </>
              )}
            </Button>
              ...
              <SheetContent side="right" className="border-amber-900/30 bg-stone-950">
                ...
                <div className="mt-8 flex flex-col gap-4">
                  <form onSubmit={handleSearch} className="md:hidden">
                    <div className="relative">
                      ...
                      <Input
                        ...
                        onChange={(e) => setLocalSearch(e.target.value)}
                        placeholder={t('common.searchPlaceholder')}
                        className="w-full h-10 border-amber-900/40 bg-stone-900/80 pl-9 pr-4 text-amber-50 text-sm placeholder:text-amber-200/50"
                        style={{ backgroundColor: 'rgba(28, 25, 23, 0.85)', color: '#fef3c7' }}
                      />
                    </div>
                  </form>
                  <button
                    ...
                    className="rounded-md px-4 py-2 text-left text-amber-200/80 transition-colors hover:bg-amber-900/20 hover:text-amber-400 flex items-center gap-2"
                  >
                    <Home className="h-4 w-4" />
                    {t('nav.home')}
                  </button>
                  ...
                  >
                    {isInstalled ? <ExternalLink className="h-5 w-5" /> : <Smartphone className="h-5 w-5" />}
                    {isInstalled ? 'Open App' : canInstall ? 'Install App' : 'Get the App'}
                  </button>
