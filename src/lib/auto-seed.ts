...
async function doSeed(): Promise<void> {
  ...
  // Fixed IDs for categories — ensures stable IDs across Vercel re-seeds
  const CATEGORY_IDS: Record<string, string> = {
    'couple-gifts': 'cat-001',
    'fashion': 'cat-002',
    'fragrances': 'cat-003',
    'home-living': 'cat-004',
    'jewelry': 'cat-005',
    'leather-goods': 'cat-006',
    'mens-shirts': 'cat-007',
    'romantic-gifts': 'cat-008',
    'sarees': 'cat-009',
    'toys': 'cat-010',
    'watches': 'cat-011',
  }
  ...
  for (const cat of CATEGORIES) {
    await db.category.upsert({
      ...
      update: {},
      create: {
        id: CATEGORY_IDS[cat.slug] || `cat-${String(CATEGORIES.indexOf(cat) + 1).padStart(3, '0')}`,
        name: cat.name,
        slug: cat.slug,
  ...
  for (const prod of PRODUCTS) {
    productIndex++
    const fixedId = `prod-${String(productIndex).padStart(3, '0')}`
    ...
    const productNumber = `PRD-${10000 + productIndex}`
  ...
  // Fixed IDs for demo users — ensures stable IDs across Vercel re-seeds
  const USER_IDS: Record<string, string> = {
    'admin@3boxesluxury.com': 'user-admin',
    'user@3boxesluxury.com': 'user-demo',
    'agent@3boxesluxury.com': 'user-agent',
    'team@3boxesluxury.com': 'user-team',
  }
  ...
  for (const user of DEMO_USERS) {
    await db.user.upsert({
      ...
      update: {},
      create: {
        id: USER_IDS[user.email] || `user-${Date.now()}`,
        email: user.email,
        name: user.name,
